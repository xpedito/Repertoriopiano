...
export const FilterBar: React.FC<FilterBarProps> = ({ 
  ...
  return (
    <div className="bg-transparent py-3 px-4 flex gap-2 overflow-x-auto no-scrollbar scroll-smooth">
      {styles.map(style => {
        const value = style === 'Todos' ? null : style;
        const isActive = selectedStyle === value;
        const isDeletable = value !== null;
            ...
            <button
              ...
              className={`whitespace-nowrap px-5 py-2.5 rounded-full text-sm font-bold transition-all border flex items-center gap-2 ${
                isActive 
                  ...
                  : 'bg-[#1c1c1e] text-gray-400 border-white/5 hover:bg-[#2c2c2e]'
              }`}
            >
              {style}
              {isDeletable && (
                <span 
                  ...
                  className={`ml-1 -mr-1 p-0.5 rounded-full transition-colors ${
                    isActive ? 'hover:bg-blue-700' : 'hover:bg-white/10'
                  }`}
                >
